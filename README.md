...
A system of many instances of one thing — an account per customer, a trade per
order — has no readable single diagram: written as one CSDF diagram, every local
control state is absorbed into a map from instance IDs to local states, and the
diagram collapses to one global state with a bundle of self-loops. So the author
writes the readable form, the diagram of *one* instance, plus a few directives,
and `csdfpromote` generates the global one.
...
```console
$ csdfpromote examples/promote/ACCOUNTS.puml
$ csdfpromote examples/promote/ACCOUNTS.puml | csdflivelockfree -
$ csdfpromote -lint-only -Werror examples/promote/TRADES.puml
```

`promote` expands a local diagram through a map; `sync` makes an event that two
kinds of instance take *together* one edge instead of two; `constrain` conjoins a
condition that reaches across maps. The start state of a local diagram means
